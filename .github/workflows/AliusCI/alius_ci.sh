...
git config pull.rebase false && git config user.email "alius@asu.edu" && git config user.name "Alius CI" && git fetch --prune &>/dev/null && git reset --hard origin/main &>/dev/null
