({
        ...
        for (let i = 0; i < contacts.length; i++) {
            if (contacts[i].portalContact.Id === contactId) {
                if (!contacts[i].isSelected) {
                    ...
                    if (!atleastOneCaseIsSelected) { // no child cases selected
    ...
    openCase: function (component, event, helper) {
        ...
        helper.openPrimaryAndSubTab(contactId, event.currentTarget.getAttribute("data-contactName"), "/" + contactId, caseId, event.currentTarget.getAttribute("data-caseNumber"), "/" + caseId, false);
    },
    ...
    openStudentProfile: function (component, event, helper) {
        ...
        helper.openPrimaryAndSubTab(contactId, contactName, "/" + contactId, profileURL, contactName + "\'s Profile", profileURL, false);
    },
